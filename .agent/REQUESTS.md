...
| 50 | claude | **기록 탭을 만들기로 했습니다(2026-08-20 · 송원석 결정).** 「연속 달성일(`streak`)·달성률」 **둘만 빼고** 나머지는 만듭니다 — 그 둘은 **추후 개선사항으로 발표**합니다. `NOW-LOG-001` 의 「연속 달성일을 만들지 않습니다」·「하지 못한 날을 세지 않습니다」는 **그대로 지킵니다**. 새 테이블은 **0개**입니다 — `[측]` 기록한 날·덜어낸 날·최빈 컨디션·자주 덜어낸 항목이 `checkins`·`evaluations`·`evaluation_results` 로 전부 계산됩니다. 나눔 — **`GET /subtract/history`(덜어내기 이력)는 제가 만들었고**, `GET /logs/summary` 확장(기록한 날·덜어낸 날·최빈 컨디션·자주 덜어낸 항목)은 김민정 님입니다 | 김민정 · 코워크 (노션 카드) |
| 51 | claude | **`GlobalExceptionHandler` 가 `MethodArgumentTypeMismatchException` 을 안 잡아 500 이 나갑니다.** `[측]` `GET /subtract/history?from=nope` → **500 `INTERNAL_ERROR`**. `Exception.class` 로 떨어집니다. **`GET /logs?from=`·`?to=`·`?limit=` 도 같은 자리에서 500 이 납니다**(김민정 님 `NOW-LOG-001`). `common/error/` 가 이철희 님 소유라 저는 `SubtractController` 안에서만 막았습니다. 전역에 한 메서드 더하시면 세 사람 것이 한 번에 해결됩니다 | 이철희 (`common/` 소유) |
