...
| 13 | claude | **JWT 무효화 수단이 없습니다.** 로그아웃해도 토큰이 만료까지 삽니다. `sessions` 테이블이 그 용도로 비어 있습니다. **동결 이후 과제**이고, 8/19 회의 안건 7의 **기본값은 「만료를 7일로 단축」**이었으나 **회의 결과 미확인**입니다. 현재 만료는 **30일**입니다 | 송원석 (회의 결과 확인) |
...
| 19 | claude | **`ItemPort` · `CheckinPort` 가 `docs/04-ports.md` 에 선언만 있고 파일이 없습니다.** `develop` 확인 결과 두 이름이 코드 어디에도 나오지 않습니다. **`ItemPort` 가 없으면 판정 API 의 ①단계(사용자가 선택한 항목만 추림)가 실제 데이터로 못 돕니다.** `CheckinPort` 가 없으면 `today/` · `home/` 이 막힙니다. **8/20 김민정 님이 이것 때문에 막혀 계신 것을 확인했습니다.** 있는 포트는 `SafetyPort` · `NoteRulePort` · `VerdictPort` 셋뿐입니다 | **`ItemPort` → 이철희** · **`CheckinPort` → 송원석** (8/17 `checkin/` 이관) |
...
| 24 | claude | **`evaluation_results.evidence_level` 을 `medium` 으로 고정해 두었습니다.** `ItemPort.SelectedItem` 이 근거 등급을 **숫자**로 넘기는데 스키마는 **문자열 `NOT NULL`** 입니다. `0`(근거 없음) 여부만 판정에 쓰이고 나머지 값은 저장에만 필요해서, 판정에 영향이 없는 `medium` 으로 채웠습니다. **`ItemPort` 실제 구현이 들어올 때 등급 문자열을 함께 넘기도록 시그니처를 넓히거나, `master/` 에서 읽어야 합니다.** 8/20 판정 API 구현 중 발견 | 이철희 (`ItemPort` 구현 시) |
...
| 26 | claude | **첫 발자국 8건의 `is_onboarding` 을 정할 근거가 어디에도 없습니다.** `schema_v63.sql:191` 과 `db_design_v63.md:150` 이 **「온보딩 노출 4건」**이라고만 적고 **어느 4건인지는 원본 어디에도 없습니다.** `jigeumbuteo_app_v6.3.html` 의 첫 발자국 화면은 8건을 전부 필터로 보여 줄 뿐 온보딩 선별이 없습니다. **지어내지 않고 8건 모두 기본값 `FALSE` 로 둡니다** — 그러면 온보딩에 아무것도 안 뜹니다. `UPDATE footsteps SET is_onboarding=TRUE WHERE id IN (…)` 한 줄이면 바뀝니다 | 송원석 (기획) |
| 27 | claude | **첫 발자국 id 형식이 문서 세 곳에서 서로 다릅니다.** 원본 `src/data/footsteps.ts` 는 **`p1`~`p8`**, `docs/07-response-rules.md:58` 은 **`fs3`**, `docs/schema_v63.sql:183` 주석은 **`fs_101`** 입니다. **관리 항목은 이런 충돌이 없습니다** — `07-response-rules.md:57` 의 `"itemId": "cr4"` 가 원본 `items.ts` 와 같고, 같은 줄의 `durationSec: 420`(=7분) 이 `cr4` 의 `mn:7` 과도 맞아 교차 확인됩니다. **첫 발자국은 김민정 님 패키지라 제가 정하지 않고 시드를 빼 두었습니다** | 김민정 · 코워크 |
...
| 34 | claude | **`SignalWeightPortStub` 이 여전히 빈 지도를 돌려줍니다.** 그래서 마스터 징후를 몇 개 고르든 `signalScore` 가 **0** 이고, 임계값 5 를 못 넘어 **상태 전환 제안이 한 번도 일어나지 않습니다.** 스텁 주석에 「시드가 들어오면 그대로 값이 흐릅니다」로 적어 뒀는데 **시드는 2026-08-20 에 들어갔습니다.** 실제 구현은 `signals` 를 읽는 것이고 **`master/` 는 김민정 님 폴더**라 제가 만들지 않았습니다. **직접 입력 징후는 각 2점이라 그쪽으로는 점수가 오릅니다** | 김민정 · 코워크 |
