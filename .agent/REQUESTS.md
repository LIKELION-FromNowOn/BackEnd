...
| 50 | claude | **상태 전환 재제안 유예를 3일로 확정했습니다(2026-08-20 · 송원석).** `NOW-STATE-003` 의 「재제안 유예 기간 미확정」과 `docs/08-open-items.md` 6번이 이 값으로 닫힙니다. `nextProposalBlockedUntil = responded_at + 3일` — **저장하지 않고 읽을 때 계산합니다.** 저장하면 기준이 바뀔 때 과거 행이 안 따라옵니다. **스키마 변경 없습니다** — `state_transitions.accepted` · `responded_at` 이 이미 있습니다. 노션 카드에도 반영했습니다 | 코워크 (`docs/08-open-items.md` 6번 닫기) |
| 51 | claude | **`ErrorCode` 두 개가 더 없습니다** — `NOW-STATE-003` 실패 코드입니다. `CHECKIN_NOT_FOUND`(404 「체크인을 찾을 수 없습니다」) · `NO_PROPOSAL`(409 「제안된 전환이 없습니다」). 인증 셋(`EMAIL_ALREADY_EXISTS`·`INVALID_CREDENTIALS`·`USER_NOT_FOUND`)과 같이 넣으시면 다섯입니다 | 이철희 (`common/` 소유) |
| 52 | claude | **`NOW-NOTE-001`·`002`(`/me/care`)를 김민정 님에게서 제가 가져갑니다(2026-08-20).** 두 API 가 `care_notes`·`care_note_lines`·`care_note_rules` **세 표를 다 읽고 씁니다 — 전부 제 엔티티**입니다. `GET` 도 `NoteRulePort` 만으로는 안 됩니다 — `lastType`(`care_notes.title`)·`ago`(`received_at`)가 포트에 없습니다. 포트를 그만큼 더 여는 것보다 제가 만드는 편이 경계가 깨끗합니다. 김민정 님은 `/me/plans` 3건 · `/logs` 2건 · `/home` **6건**이 남습니다 | 기록 (조치 완료) |
